...
async def client_to_agent_messaging(
    websocket: WebSocket, live_request_queue: LiveRequestQueue,session_id
):
    ...
    while True:
        ...
        if mime_type == "text/plain":
            ...
            stored_session = session_service.sessions[APP_NAME][session_id][session_id]
            #send the whole collected data of the latest index and ask further if and only if needed
            state:QA = stored_session.state['state']
            state.question.append(data)
            stored_session.state['state'] = state
